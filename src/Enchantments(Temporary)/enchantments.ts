...
class EnchDisplayedParameter {
    ...
    static calcSkillMult = (skill: number): number => 0/0; // placeholder
    calc = (skillMult: number): number => 0/0; // placeholder

    // function to rewrite the content of the element to new value
    update(skillMult: number) {
        this.elementText = String(this.calc(skillMult));
    }
    // updates a set (use from specific class) using update()
    static updateSet(parameters: Set<EnchDisplayedParameter>) {
...
class EnchMagnitude extends EnchDisplayedParameter {
    _base: string;
    _zero: number;
    _growthMult: number = 0; // placeholder
...
export class ArmoMag extends EnchMagnitude {
    ...
    static calcSkillMult = (skill: number) => Math.pow(skill/100, 2);
}
...
export class WeapMag extends EnchMagnitude {
    ...
    static calcSkillMult = (skill: number) => Math.pow((skill/100),(1 + Math.sqrt(skill*fEnchantingSkillCostBase)));
...
class WeapBaseEnch extends EnchDisplayedParameter {
...
export class WeapCharges extends WeapBaseEnch {
    ...
    static calcSkillMult = (skill: number) => one_min_sqrt.calc(skill);
...
export class WeapPrice extends WeapBaseEnch {
    ...
    static calcSkillMult = (skill: number) => 
        one_min_sqrt.calc(skill) // (1 - Math.sqrt(skill * fEnchantingSkillCostBase))
        * fEnchantmentEffectPointsMult;
